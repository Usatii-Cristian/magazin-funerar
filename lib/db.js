...
function normalize(p) {
  const imgs = (p.images?.length ? p.images : p.image ? [p.image] : []).filter(
...
export async function getProducts() {
  // Static catalog is a fallback ONLY — once the DB has any products,
  // those become the source of truth. Otherwise deleting a product whose
  // name overlaps with the demo catalog would be silently undone by the
  // dedupe-and-merge that lived here previously.
  try {
